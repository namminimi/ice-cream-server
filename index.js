...
app.post("/join", async (req, res)=> {
    ...
    let myPass = "";
    const {m_name, m_password, m_id, m_nickname, m_birth, m_gender, m_phone, m_add1, m_add2, m_comnick, m_passwordCh} = req.body
    console.log(11111)
    if(mytextpass != "" && mytextpass != undefined){
        ...
        bcrypt.genSalt(saltRounds, function(err, salt){
            bcrypt.hash(mytextpass, salt, function(err, hash){
                myPass = hash;
                conn.query(`insert into member(m_name, m_password, m_id, m_nickname, m_birth, m_gender, m_phone, m_add1, m_add2, m_comnick, m_passwordch) values('${m_name}','${myPass}','${m_id}','${m_nickname}','${m_birth}','${m_gender}','${m_phone}','${m_add1}','${m_add2}','${m_comnick}','${m_passwordCh}')`
                ,(err, result, fields)=>{
                    console.log(result)
                    if(result){
                        console.log("회원가입성공")
...
app.post("/writeNotice", async (req,res)=>{
    ...
    conn.query(`insert into notice(w_title, w_desc, w_date,w_username) values('${w_title}','${ w_desc}','${w_date}','${w_username}')`,
    (err,result,fields)=>{
        console.log(result)
        if(result){
            console.log(result)
...
app.get("/notice/:no", async (req,res)=>{
    const {no} = req.params
    conn.query(`select * from notice where w_no=${no}`,
    (err, result, fields) => {
        if(result){
            //console.log(result)
            res.send(result)
...
app.patch("/editNotice", async (req,res)=>{
    ...
    console.log(req.body)
    console.log(w_no)
    conn.query(`update notice set w_title='${w_title}', w_desc='${w_desc}', w_username='${w_username}',w_date='${w_date}' where w_no= '${w_no}'`,
    (err, result, fields)=>{
        if(result){
            //console.log(result)
            res.send(result)
...
app.delete('/deleteNotice/:no', async (req, res)=>{
    const {no} = req.params
    conn.query(`delete from notice where w_no = '${no}'`,
    (err, result, fields)=>{
        if(result){
            res.send("삭제되었습니다.")
            console.log(result)
...
app.post('/carts', async (req,res)=>{
    const {c_userId, c_title,c_price, c_taste, c_amount, c_img,c_point, c_brand} = req.body
    conn.query(`insert into cart(c_userId, c_title,c_price, c_taste, c_amount, c_img, c_point,c_brand) values('${c_userId}','${c_title}','${c_price}','${c_taste}','${c_amount}','${c_img}','${c_point}','${c_brand}')`,
    (err, result, fields)=>{
        if(result){
            console.log(result)
            res.send(result)
...
app.delete('/delCart/:num', async (req, res)=> {
    ...
    console.log(num)
    conn.query(`delete from cart where c_no=${num}`,
    (err,result,fields)=>{
        if(result) {
            console.log("삭제되었습니다.")
            res.send("삭제되었습니다")
...
app.get('/personal/:id' , async (req, res)=> {
    const {id} = req.params
    conn.query(`select * from member where m_id='${id}'`,
    (err, result, fields) => {
        if(result){
            console.log(result)
            res.send(result[0])
...
app.post('/finalCheck', async (req, res) => {
    const {ok_name, ok_nickname, ok_phone, ok_address1, ok_address2, ok_id} = req.body
    console.log(req.body)
    conn.query(`update member set m_name='${ok_name}', m_nickname='${ok_nickname}',m_phone='${ok_phone}',m_add1='${ok_address1}',m_add2='${ok_address2}' where m_id='${ok_id}'`,
    (err, result, fields)=>{
        if(result){
            console.log(result)
            res.send("수정완료")
...
app.post('/end', async (req,res)=>{
    ...
    conn.query(`select * from member where m_id='${userId}' and m_passwordch='${password}'`,
    (err, result, fields)=>{
        if(result){
            console.log(result)
            res.send(result[0])
...
app.delete('/deleteMember/:userId', async (req, res)=>{
    ...
    console.log(1111)
    conn.query(`delete from member where m_id='${userId}'`,
    (err, result, fields)=>{
        if(result){
            console.log(result)
            res.send(result)
...
app.post('/addIce', async(req, res) => {
    ...
    if(p_img2 === "" && p_img3 === "" && p_img4 === ""){
        ...
        img3 = null
        img4 = null
        conn.query(`insert into products(p_title, p_dsce, p_img1, p_img2, p_img3, p_img4, p_brand, p_taste, p_price, p_amount, p_point) values('${p_title}', '${p_dsce}', '${p_img1}', ${img2}, ${img3}, ${img4}, '${p_brand}', '${p_taste}', '${p_price}', '${p_amount}', '${p_point}')`,
        (err, result, fields) => {
        if(result){
            console.log(result)
            res.send("등록되었습니다.")
